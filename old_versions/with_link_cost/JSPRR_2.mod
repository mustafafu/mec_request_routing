...
# Objective Function
maximize z:  sum {f in U} S_W[U_S[f]] * ( sum {g in N} x[g,f]) - sum {f in U} (sum{h in E} u[f,h] * O[h] * link_load[h]);# Deciding if user will be served
...
s.t. linkLoadConstraint {ll in E}:
	sum {ii in U} u[ii,ll] * S_H[ U_S[ii] ] = link_load[ll]; 
